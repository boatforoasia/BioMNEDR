...
const int vocab_hash_size = 300000000;  // Maximum 80 * 0.7 = 56M words in the vocabulary
...
int hs = 0, negative = 5;
const int table_size = 1e9; // const int table_size = 1e8;
int *table;


//added 
std::vector<long long> d_table(0), i_table(0), p_table(0), s_table(0);
...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    if (vocab[target].word[0] == 'd') { // v -> d
      d_table.push_back(target);    // v_table -> d_table
    } else if (vocab[target].word[0] == 'i') {
      i_table.push_back(target);    // i_table -> D_table
    } else if (vocab[target].word[0] == 'p') {
      p_table.push_back(target);    // f_table -> p_table
    } else if (vocab[target].word[0] == 's') {
      s_table.push_back(target);    // a_table -> s_table
    }
  }
  ...
  std::cout << "s_table.size() " << s_table.size() << std::endl;
}
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
          if (d == 0) {
            ...
            if (pp) {
              ...
              } else if (vocab[last_word].word[0] == 's') {
                target = s_table[(next_random >> 16) % s_table.size()];
