...
class MSI():
	...
	def load_proteins_in_graph(self):
		self.proteins_in_graph = list(self.type2nodes[PROTEIN])

	def load_functions_in_graph(self):
		self.functions_in_graph = list(self.type2nodes['biological_function'])
	...
	def load_drug2proteins(self):
		drug2proteins = dict.fromkeys(self.drugs_in_graph)
		for drug in self.drugs_in_graph:
			assert (not (nx.is_directed(self.components["drug_to_protein"].graph)))
			drug2proteins[drug] = set(self.components["drug_to_protein"].graph.neighbors(drug))
		self.drug2proteins = drug2proteins

	def load_protein2drugs(self):
		protein2drugs = dict.fromkeys(self.proteins_in_graph)
		for protein in self.proteins_in_graph:
			if protein in self.components["drug_to_protein"].graph.nodes:
				protein2drugs[protein] = set(self.components["drug_to_protein"].graph.neighbors(protein))
			else:
				protein2drugs[protein] = set()
		self.protein2drugs = protein2drugs

	def load_protein2proteins(self):
		protein2proteins = dict.fromkeys(self.proteins_in_graph)
		for protein in self.proteins_in_graph:
			assert (not (nx.is_directed(self.components["protein_to_protein"].graph)))
			protein2proteins[protein] = set(self.components["protein_to_protein"].graph.neighbors(protein))
		self.protein2proteins = protein2proteins

	def load_protein2functions(self):
		protein2functions = dict.fromkeys(self.proteins_in_graph)
		for protein in self.proteins_in_graph:
			if protein in self.components["protein_to_functional_pathway"].graph.nodes:
			# assert (not (nx.is_directed(self.components["protein_to_functional_pathway"].graph)))
				protein2functions[protein] = set(self.components["protein_to_functional_pathway"].graph.neighbors(protein))
			else:
				protein2functions[protein] =set()
		self.protein2functions = protein2functions

	def load_function2functions(self):
		function2functions = dict.fromkeys(self.functions_in_graph)
		for function in self.functions_in_graph:
			# assert (not (nx.is_directed(self.components["functional_pathway_to_functional_pathway"].graph)))
			function2functions[function] = set(self.components["functional_pathway_to_functional_pathway"].graph.neighbors(function))
		self.function2functions = function2functions

	def load_function2proteins(self):
		function2proteins = dict.fromkeys(self.functions_in_graph)
		for function in self.functions_in_graph:
			if function in self.components["protein_to_functional_pathway"].graph.nodes:
			# assert (not (nx.is_directed(self.components["protein_to_functional_pathway"].graph)))
				function2proteins[function] = set(self.components["protein_to_functional_pathway"].graph.neighbors(function))
			else:
				function2proteins[function] = set()
		self.function2proteins = function2proteins

	def load_indication2proteins(self):
		indication2proteins = dict.fromkeys(self.indications_in_graph)
		for indication in self.indications_in_graph:
			assert(not(nx.is_directed(self.components["indication_to_protein"].graph)))
			indication2proteins[indication] = set(self.components["indication_to_protein"].graph.neighbors(indication))
		self.indication2proteins = indication2proteins

	def load_protein2indications(self):
		protein2indications = dict.fromkeys(self.proteins_in_graph)
		for protein in self.proteins_in_graph:
			if protein in self.components["indication_to_protein"].graph.nodes:
				protein2indications[protein] = set(self.components["indication_to_protein"].graph.neighbors(protein))
			else:
				protein2indications[protein] = set()
		self.protein2indications = protein2indications
	...
	def load(self):
		...
		self.load_proteins_in_graph()
		self.load_functions_in_graph()
		self.load_drug_or_indication2proteins()
		self.load_drug2proteins()
		self.load_protein2drugs()
		self.load_protein2proteins()
		self.load_function2proteins()
		self.load_protein2functions()
		self.load_function2functions()
		self.load_protein2indications()
		self.load_indication2proteins()
